...
void main(void) {
    ...
    config1 = ADC_FOSC_8 | ADC_RIGHT_JUST | ADC_2_TAD;
    config2 = ADC_CH0 | ADC_INT_ON | ADC_REF_VDD_VSS;
    //    //setting all ports as analog
    portconfig = ADC_0ANA;
    ...
    while (1) {
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                case MSGT_I2C_RCV:
                {
                    // TODO Send data via UART to Master PIC
                    //                    PIE1bits.TX1IE = 0x1; //set UART TXF interrupt flag
                    //                    ToMainHigh_sendmsg(length, msgtype, void * msgbuffer);
                };
                case MSGT_I2C_RQST:
                {
                    // TODO Send I2C through main
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                case MSGT_UART_RCV:
                {
                    //TODO figure out how where to send message
                    uart_lthread(&uthread_data, msgtype, length, msgbuffer);
                    break;
                };
                default:
